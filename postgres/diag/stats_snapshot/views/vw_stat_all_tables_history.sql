CREATE OR REPLACE VIEW vw_stat_all_tables_history AS
WITH stat_history AS (
  ...
  -- SELECT distinct on(schemaname, relname) * FROM  stat_final sf WHERE change_ratio > 100 order by schemaname, relname asc, inserted_at desc -- show only one result (the last) per table
   SELECT * 
     FROM stat_final
    WHERE NOT (seq_scan_df = 0 and seq_scan = 0 and seq_tup_read_df = 0 and seq_tup_read = 0 and idx_scan_df = 0 
                and idx_scan = 0 and idx_tup_fetch_df = 0 and idx_tup_fetch = 0 and n_tup_ins_df = 0 and n_tup_ins = 0 
                and n_tup_del_df = 0 and n_tup_del = 0 and n_tup_upd_df = 0 and n_tup_upd = 0 and n_tup_hot_upd_df = 0  
                and n_tup_hot_upd = 0 and n_live_tup_df = 0 and n_live_tup = 0 and n_dead_tup_df = 0 and n_dead_tup = 0
                and n_mod_since_analyze_df = 0 and n_mod_since_analyze = 0 and vacuum_count_df = 0 and vacuum_count = 0 )
     ORDER BY inserted_at ASC, schemaname asc, relname asc -- show all results (all tables and its encarnations)
